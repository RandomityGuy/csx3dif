...
impl Readable<Interior> for Interior {
    fn read(from: &mut dyn Buf, version: &mut Version) -> DifResult<Self> {
        ...
        let indices = read_vec::<PointIndex, u16>(from, version, |p, _| p, |x| PointIndex::new(x as _))?;
        ...
        let normal_indices = if version.interior >= 4 && version.interior <= 5 {
            read_vec::<NormalIndex, u8>(from, version, |alt, p| alt && p == 0, |x| NormalIndex::new(x as _))?
        } else {
            vec![]
        };
        ...
        let hull_indices = read_vec::<PointIndex, u16>(from, version, |alt, _| alt, |x| PointIndex::new(x as _))?;
        let hull_plane_indices = read_vec::<PlaneIndex, u16>(from, version, |_, _| true, |x| PlaneIndex::new(x as _))?;
        let hull_emit_string_indices = read_vec::<EmitStringIndex, u16>(from, version, |alt, _| alt, |x| EmitStringIndex::new(x as _))?;
        let hull_surface_indices = read_vec::<PossiblyNullSurfaceIndex, u16>(from, version, |alt, _| alt, |x| PossiblyNullSurfaceIndex::from(x as u32))?;
        let poly_list_plane_indices = read_vec::<PlaneIndex, u16>(from, version, |_, _| true, |x| PlaneIndex::new(x as _))?;
        let poly_list_point_indices = read_vec::<PointIndex, u16>(from, version, |alt, _| alt, |x| PointIndex::new(x as _))?;
        ...
        let coord_bin_indices = read_vec::<ConvexHullIndex, u16>(from, version, |_, _| true, |x| ConvexHullIndex::new(x as _))?;
...
impl Writable<Interior> for Interior {
    fn write(&self, to: &mut dyn BufMut, version: &Version) -> DifResult<()> {
        ...
        if version.interior >= 13 {
            ...
            write_vec_fn::<LMapIndex, u8>(&self.normal_lmap_indices, to, version, |i| *i.inner() as u8)?;
        }
        if version.interior >= 13 {
            self.alarm_lmap_indices.write(to, version)?;
        } else if version.interior != 4 {
            write_vec_fn::<LMapIndex, u8>(&self.alarm_lmap_indices, to, version, |i| *i.inner() as u8)?;
        }
